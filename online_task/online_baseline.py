...
def load_embeddings(path):
    ...
    nontoxic_toxicities = np.array([toxicity.get(word, 1.0) for word, ind in embs_voc.items() if toxicity.get(word, 1.0) <= 0.5])
    ...
    return embs_vectors, embs_voc, embs_voc_by_id, nontoxic_emb_inds, embs_vectors_normed_nontoxic, embs_tree, nontoxic_toxicities
...
def find_closest_nontoxic(word, threshold=0.5, allow_self=False):
    ...
    query = embs_tree.query(word_emb, k=10, return_distance=True)
    distances, indexes = np.array(query[0][0]), np.array(query[1][0])
    toxicities = nontoxic_toxicities[indexes]
    scaled_query = sorted(list(zip(distances * toxicities, indexes)), key=lambda x: x[0])
    i = scaled_query[0][1]
...
def detox(line):
    line = normalize(line)
    output = tokenizer(line)
    input_ids, attention_mask = torch.tensor([output["input_ids"][:512]]), torch.tensor(
        [output["attention_mask"][:512]])

    logits = model(input_ids=input_ids, attention_mask=attention_mask).logits
    sentense_toxicity = torch.sigmoid(logits)[:, -1].detach().numpy()

    if sentense_toxicity < 0.5:
        return line
...
if __name__ == '__main__':
    ...
    # load embeddings
    embs_vectors, embs_voc, embs_voc_by_id, nontoxic_emb_inds, embs_vectors_normed_nontoxic, embs_tree, nontoxic_toxicities = load_embeddings(
        args.embeddings)
